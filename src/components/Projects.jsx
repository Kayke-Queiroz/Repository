...
const ProjectCard = ({ project, index, x, t }) => {
    ...
    const opacity = useTransform(dist, (d) => {
        // Ajustado para manter os cards visíveis por mais tempo antes de piscar na borda vazia
        // Se TOTAL_ITEMS for pequeno (ex: 4), queremos manter a opacidade alta
        const maxDistance = TOTAL_WIDTH / (TOTAL_ITEMS >= 5 ? 3 : 2.5);
        ...
        >
            ...
            <div className="p-6 flex flex-col h-full relative z-10 pt-10">
                ...
                <div className="mt-auto flex gap-2">
                    {/* Botão Ver Projeto */}
                    {projectStatic.url === "#" ? (
                        <div className="flex-1 py-2.5 rounded-lg bg-white/5 border border-white/10 text-gray-500 font-bold flex items-center justify-center gap-2 text-xs cursor-not-allowed">
                    ...
                    {/* Botão GitHub */}
                    {projectStatic.github === "#" ? (
                        <div className="flex-1 py-2.5 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center gap-2 text-xs font-bold text-gray-500 cursor-not-allowed">
                        ...
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex-1 py-2.5 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center gap-2 text-xs font-bold text-white transition-all duration-300 hover:bg-white/10 hover:border-white/20 active:scale-95 cursor-pointer"
                            onPointerDown={(e) => e.stopPropagation()}
                        >
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" /></svg>
                            {t.projects.github}
